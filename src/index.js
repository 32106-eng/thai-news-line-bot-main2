...
function advice(monthTransactions) {
  ...
  if (expenses.length < 3) return "คำแนะนำ: บันทึกรายการต่อเนื่องอีกนิด ตาผินจะช่วยดูพฤติกรรมการใช้เงินให้";
...
<html lang="th"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>รายการ | ตาผิน</title>
<style>
...
app.post("/webhook", express.raw({ type: "application/json" }), async (req, res) => {
  ...
  for (const event of payload.events ?? []) {
    ...
    let message;
    if (["เริ่ม", "ช่วยเหลือ", "help"].includes(text.toLowerCase())) message = "📒 ตาผินพร้อมจดบัญชีของคุณ (ข้อมูลของแต่ละคนแยกกันเป็นส่วนตัว)\n\nพิมพ์: กาแฟ 60\nรายรับ: เงินเดือน 15000\nถ่ายรูปใบเสร็จส่งมาได้เลย (ฟีเจอร์ Premium) ตาผินจะอ่านยอดกับร้านค้าให้อัตโนมัติ\nคำสั่ง: สรุปวันนี้ | สรุปเดือนนี้ | ลบล่าสุด | เว็บ | สมัครพรีเมียม\nทุกวันอาทิตย์ตาผินจะสรุปสัปดาห์ให้อัตโนมัติด้วย\n\nหรือถามตาผินได้เลย เช่น \"เดือนนี้ใช้เงินไปกับอะไรมากสุด\" ตาผินเน้นตอบเรื่องการเงินเป็นหลัก แต่คุยเรื่องอื่นได้ด้วย";
...
async function askFinanceAi(user, question) {
  ...
  try {
    ...
    const completion = await ai.chat.completions.create({
      ...
      temperature: 0.4,
      messages: [
        { role: "system", content: "คุณคือ \"ตาผิน\" ผู้ช่วยส่วนตัวทาง LINE เน้นให้คำปรึกษาและตอบคำถามด้านการเงินส่วนบุคคล (การออม การใช้จ่าย การตั้งงบประมาณ หนี้สิน หลักการลงทุนเบื้องต้น) โดยใช้ข้อมูลบัญชีของผู้ใช้ที่ให้มาประกอบการตอบเมื่อเกี่ยวข้อง คุณตอบคำถามทั่วไปอื่น ๆ นอกเรื่องการเงินได้เช่นกันแบบสั้นและเป็นมิตร แต่เมื่อมีโอกาสให้โยงกลับมาช่วยเรื่องการเงินอย่างเป็นธรรมชาติ ตอบเป็นภาษาไทย กระชับ ไม่เกิน 4 ประโยค เหมาะสำหรับส่งทางแชท ห้ามให้คำแนะนำการลงทุนเฉพาะเจาะจงที่มีความเสี่ยงสูงหรือรับประกันผลตอบแทน และห้ามให้คำแนะนำทางกฎหมายหรือภาษีแบบฟันธง ให้แนะนำปรึกษาผู้เชี่ยวชาญแทนในกรณีนั้น" },
        { role: "user", content: `${context}\n\nคำถามจากผู้ใช้: ${question}` }
      ]
    });
    return completion.choices[0]?.message?.content?.trim() || null;
