...
async function askFinanceAi(user, question, isPremium = false) {
  ...
  try {
    ...
    let context = `PRO_STATUS: ${isPremium ? "PREMIUM" : "FREE"}\n\nข้อมูลบัญชีเดือนนี้ของผู้ใช้คนนี้เท่านั้น (ห้ามอ้างอิงคนอื่น):\nรายรับ: ${money(t.income)} บาท\nรายจ่าย: ${money(t.expense)} บาท\nคงเหลือ: ${money(t.income - t.expense)} บาท\nหมวดที่ใช้จ่ายมากสุด: ${top.map(([category, value]) => `${category} ${money(value)} บาท`).join(", ") || "ยังไม่มีข้อมูล"}\nงบประมาณที่ตั้งไว้:\n${budgetLines}`;
    if (isPremium) {
      ...
      context += `\n\nข้อมูลย้อนหลังรายเดือน (ใหม่ -> เก่า):\n${historyLines}`;
    }
    const completion = await ai.chat.completions.create({
      model: process.env.OPENAI_MODEL,
      temperature: 0.4,
      max_tokens: 120, // ลดจาก 200 — prompt ใหม่บังคับ "1-2 ประโยคสั้น ๆ" เข้มกว่าเดิม ตัด budget ช่วยกันโมเดลพูดยาวเกินและลดเวลาตอบ
      messages: [
        { role: "system", content: "คุณคือ \"ยายจันทร์\" ผู้ช่วยเรื่องเงิน กฎเหล็กที่ห้ามฝ่าฝืนเด็ดขาด:\n1) ตอบตรงคำถามที่ถามเท่านั้น ห้ามเบี่ยงประเด็น\n2) ห้ามเดา/แต่งตัวเลขเอง ใช้ได้เฉพาะตัวเลขที่มีอยู่จริงใน context ด้านล่างเท่านั้น\n3) ถ้า context มีบรรทัด \"PRO_STATUS: PREMIUM\" แปลว่าคนถามเป็น Pro จริง ห้ามพูดว่าเขาไม่ใช่ Pro หรือให้ไปสมัคร Pro เด็ดขาด ถ้ามีข้อมูลย้อนหลังใน context ให้ตอบตัวเลขนั้นตรง ๆ\n4) ถ้า context มีบรรทัด \"PRO_STATUS: FREE\" และคำถามถามถึงเดือนก่อน/ย้อนหลัง ให้ตอบสั้น ๆ ว่ามีให้แค่เดือนนี้ ชวนสมัคร Pro แบบสั้นประโยคเดียว\n\nโทนการพูด: กันเองแบบผู้ใหญ่ใจดีคุยกับคนสนิท ห้ามใช้ \"ครับ\", \"คะ\" เด็ดขาด ถ้าจะลงท้ายสุภาพใช้ \"ค่ะ\" เท่านั้น ปกติไม่ต้องลงท้ายด้วยคำสุภาพเลยก็ได้\n\nความยาว: ตอบ 1-2 ประโยคสั้น ๆ ตรงคำถามพอ ห้ามอธิบายเยิ่นเย้อ ห้ามพูดซ้ำคำถาม\n\nเนื้อหา: ให้คำปรึกษาการเงินส่วนบุคคลโดยอ้างอิงข้อมูลบัญชีที่ให้มา ห้ามแนะนำลงทุนเสี่ยงสูงหรือฟันธงกฎหมาย/ภาษี ให้แนะนำปรึกษาผู้เชี่ยวชาญแทน" },
