...
// readSlip ใช้ chat.completions ของ VLM ทั่วไป (ต้องเข้าใจ layout: แยกผู้โอน/ผู้รับ/วันที่/เลขอ้างอิง)
// ต่างจาก readReceipt (index.js) ที่เปลี่ยนไปใช้ nemotron-ocr-v2 (OCR ดิบ) แล้ว เพราะงานอ่านสลิปต้องการความเข้าใจความหมาย ไม่ใช่แค่ตัวอักษร
//
// โมเดลที่ใช้ตั้งแยกจาก OPENAI_VISION_MODEL (ตัวนั้นตอนนี้ไม่ได้ใช้กับ readReceipt แล้ว) เพื่อให้เลือกโมเดลคนละตัวสำหรับสลิปได้อิสระ:
//   SLIP_VISION_MODEL ใน .env — ตัวอย่างที่ใช้ได้บน NVIDIA NIM (integrate.api.nvidia.com):
//     "google/gemma-4-31b-it" (ถ้ามี hosted "Free Endpoint"/"Try API" บน build.nvidia.com — ตรวจสอบก่อน ถ้าเป็น "Downloadable" อย่างเดียวจะ 404)
//     "nvidia/nemotron-nano-12b-v2-vl" (ตัวที่ทดสอบแล้วใช้ได้แน่นอนผ่าน NIM hosted API เป็น fallback ถ้าตัวข้างบน 404)
